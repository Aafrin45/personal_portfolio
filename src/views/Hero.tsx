
import downloadIcon from "../assets/download-btn-icon.svg";

// components
import { Button } from "../components";

// react-simple-typewriter
import { Typewriter } from "react-simple-typewriter";

// framer-motion
import { motion } from "framer-motion";


const Hero = () => {
  return (
    <div
      id="home"
      className="min-screen flex items-center justify-center relative"
      style={{
        backgroundColor:"#222831",
      }}
    >
      <div
        className="max-w-screen-2xl flex flex-col 
          xl:flex-row xl:justify-between items-center xl:items-start gap-12 w-full py-16 px-12"
      >
        <div className="w-full xl:w-fit">
          <motion.h1
            initial="hidden"
            whileInView="visible"
            viewport={{ once: false }}
            className="w-full xl:w-fit text-center xl:text-start text-4xl sm:text-6xl lg:text-8xl
              font-bolt text-textPrimary uppercase"
          >
            I'm Aafrin Sulaiha
            <br />
            <span className="text-secondary">
              <Typewriter
                words={["Developer", "Designer",]}
                cursor
                cursorStyle="_"
                typeSpeed={250}
                deleteSpeed={150}
                loop
              />
            </span>
          </motion.h1>
          <motion.div
            initial="hidden"
            whileInView="visible"
            viewport={{ once: false }}
            className="my-12 flex flex-col sm:flex-row items-center gap-6 justify-center xl:justify-start"
          >
            <Button linkTo="mailto:aafrinsulaiha@gmail.com" secondary>Hire me</Button>
            <Button linkTo="https://drive.google.com/file/d/1Zlf17Lwu5d01XRagfukyb5JWURmFNXq6/view" icon={downloadIcon}>Download CV </Button>
          </motion.div>
        </div>
      </div>

      <div className="absolute bottom-0 left-0 w-full h-[1px] " />
    </div>
  );
};

export default Hero;
